...
type dockerImageSource struct {
	ref dockerReference
	c   *dockerClient
	t   *torrent.TorrentClient
	// State
	cachedManifest         []byte // nil if not loaded yet
	cachedManifestMIMEType string // Only valid if cachedManifest != ni
...
func newImageSource(sys *types.SystemContext, ref dockerReference) (*dockerImageSource, error) {
	...
	if sys.DockerTryTorrent {
		t, err = torrent.MakeTorrentClient(sys, logrus.GetLevel() == logrus.DebugLevel, false)
		if err != nil {
			return nil, err
		}
	}
